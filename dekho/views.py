        ...
        try:
            ...
            if "product_list" in data.keys():
                ...
                for row in rows[:len(rows)-1]:
                    row = row.split('\\t')
                    mails.append(row[1])
                    data_mail = {'email_body': f'Congratulations! You have been awarded with a coupon having code {k}',
                                 'email_subject': 'Coupon Code', 'to_email': row[1]}
                ...
                if serializer.is_valid(raise_exception=True):
                    ...
                    if data["applicable_to"] == "specific_sku":
                        ...
                        products = Product.objects.filter(
                            sku__in=data["applicable_sku"])
                        for product in products:
                            cp = CouponProductSerializer(
                                data={"coupon": coupon.id, "product": product.id})
        ...
        try:
            ...
            for name in names:
                product = Product.objects.filter(
                    name__icontains=name).values_list("company", flat=True)
                coupon = Coupon.objects.filter(user__name__icontains=name)
                serializer = CouponSerializer(coupon, many=True)
                coupons.extend(list(serializer.data))
                coupon = Coupon.objects.filter(user__in=product)
                serializer = CouponSerializer(coupon, many=True)
                coupons.extend(list(serializer.data))
            return Response({"status": True, "data": coupons, "message": 'Success'}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"status": False, "data": {}, "message": f"{e}"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            for row in rows[:len(rows)-1]:
                ...
                if st:
                    row[0] = row[0][2:]
                    st = 0
                product = Product(company=request.user, name=row[0], description=row[1],
                                  price=row[2], quantity=row[3], sku=row[4][:len(row[4])-2], image_url=row[5])
                product.save()
            products = Product.objects.filter(company=request.user)
        ...
        try:
            ...
            for name in names:
                company = User.objects.filter(
                    name__icontains=name).values_list("email", flat=True)
    ...
    def delete(self, request):
        try:
            ...
            orderitem = OrderItem.objects.get(
                order=cart, product=data["product"])
            cart.total_amount -= (orderitem.price * orderitem.quantity)
            cart.save()
            orderitem.delete()
            return Response({"status": True, "data": {}, "message": 'Success'}, status=status.HTTP_200_OK)
...
def validate_coupon(request):
    try:
        ...
        skus = Product.objects.filter(
            sku__in=request.data["skus[]"]).values_list('sku', flat=True)
        coupon = Coupon.objects.get(code=data["coupon"])
        rules = [CouponRule(coupon.redemption_limit, coupon.expiry_date, coupon.active,
                            coupon.used, coupon.applicable_to), CartRule(list(skus), coupon.applicable_to)]
        pc = CouponProduct.objects.filter(
            coupon=coupon).values_list('product', flat=True)
        subject = {"skus": list(Product.objects.filter(
            id__in=list(pc)).values_list('sku', flat=True))}
        evaluation = run(subject, rules)
        data = {"valid": evaluation.result, "discount": coupon.discount_value,
                "type": coupon.discount_type, "max_discount": coupon.max_discount_amount}
...
def company_coupons(request):
    try:
        ...
        company = User.objects.filter(
            name__icontains=data).values_list('email', flat=True)
